defmodule Websocket.Exchanges.Client do
  use GenServer
  @type t :: %__MODULE__{
    module: module(),
    conn: pid(),
  ...
  defstruct [:module, :conn, :con_ref, :currency_pairs]
  ...
  def init({module, currency_pairs}) do
    client = %__MODULE__{module: module, currency_pairs: currency_pairs}
  ...
  def validate_required(msg, keys) do
    required_key = Enum.find(keys, fn k ->
      is_nil(msg[k])
    end)
