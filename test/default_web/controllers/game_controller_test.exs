defmodule DefaultWeb.GameControllerTest do
  use DefaultWeb.ConnCase

  alias Default.Games

  @create_attrs %{month: "some month", name: "some name", status: "some status"}
  @update_attrs %{month: "some updated month", name: "some updated name", status: "some updated status"}
  @invalid_attrs %{month: nil, name: nil, status: nil}
  ...
  describe "update game" do
    ...
    test "redirects when data is valid", %{conn: conn, game: game} do
      ...
      assert html_response(conn, 200) =~ "some updated month"
    end
